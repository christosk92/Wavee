...
namespace Wavee.Core.Decoders.VorbisDecoder.Infrastructure.Checksum;

public class OggCrc32 : IMonitor
{
    private static uint[][] CRC32TABLE = new uint[8][]
     {
        new uint[]
        {
    ...
    // RefOut = false
    // XorOut = false
    public OggCrc32(uint initialState)
    {
        state = initialState;
    }
    ...
    {
        state = (state << 8) ^ CRC32TABLE[0][((state >> 24) ^ b) & 0xFF];
        //state = (state << 8) ^ CRC32TABLE[0][(state >> 24) ^ b];
    }

    public void ProcessBufferBytes(Span<byte> buf)
    {
        int index = 0;
        ...
        while (index + 8 <= buf.Length)
        {
            ...
            state = CRC32TABLE[7][(xor >> 24) & 0xFF] ^
                    CRC32TABLE[6][(xor >> 16) & 0xFF] ^
                    CRC32TABLE[5][(xor >> 8) & 0xFF] ^
                    CRC32TABLE[4][(xor) & 0xFF] ^
                    CRC32TABLE[3][buf[index + 4]] ^
                    CRC32TABLE[2][buf[index + 5]] ^
                    CRC32TABLE[1][buf[index + 6]] ^
                    CRC32TABLE[0][buf[index + 7]];
