...
public class TrackOrEpisode
{
    public TrackOrEpisode(Either<Episode, Lazy<Track>> Value)
    {
        ...
        // {
        //     var e = Value.left;
        //     Id = new AudioId(AudioItemType.Episode, Encode(e.Uri));
        // }
        // else
        // {
        //     var t = Value.RightValue;
        //     Id = new AudioId(AudioItemType.Track, Encode(t.Value.Uri));
        // }
    }
    ...
    public Episode AsEpisode => Value.Match(
        Left: e => e,
        Right: t => throw new InvalidOperationException("This is an track")
    );
    ...
    public string? GetImageAsIndex(int index)
    {
        return GetImage((Image.Types.Size)index);
    }
    public string GetImage(Image.Types.Size size)
    {
        ...
        var res = Value.Match(
                ...
                Right: t =>
                {
                    ...
                    if (r is not null)
                        return Some(r);
                    if (t.Value.Album.CoverGroup.Image.Count == 0)
                        return None;
                    return Some(t.Value.Album.CoverGroup.Image[0]);
                })
            .Bind(img =>
            {
        ...
        return res.IfNone(string.Empty);
    }
