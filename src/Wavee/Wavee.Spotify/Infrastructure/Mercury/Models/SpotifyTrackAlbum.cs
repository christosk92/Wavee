...
public readonly partial record struct SpotifyTrackAlbum(AudioId Id, string Name, Seq<Artwork> Artwork,
    DateOnly ReleaseDate, ReleaseDatePrecisionType ReleaseDatePrecision, int DiscNumber,
    string ArtistName) : IComparable<SpotifyTrackAlbum>
{
    public static SpotifyTrackAlbum From(Show show)
    {
        return new SpotifyTrackAlbum(
            Id: AudioId.FromRaw(show.Gid.Span, AudioItemType.PodcastShow, ServiceType.Spotify),
            Name: show.Name,
            Artwork: show.CoverImage?.Image?.Select(x => ToArtwork(x))?.ToSeq() ?? LanguageExt.Seq<Artwork>.Empty,
    ...
    public static SpotifyTrackAlbum From(Album album, int discNumber)
    {
        ...
        return new SpotifyTrackAlbum(
            ...
            Name: album.Name,
            Artwork: album.CoverGroup?.Image?.Select(x => ToArtwork(x))?.ToSeq() ?? LanguageExt.Seq<Artwork>.Empty,
    ...
    private static Artwork ToArtwork(Image image)
    {
        const string cdnUrl = "https://i.scdn.co/image/{file_id}";
        var imageId = ToBase16(image.FileId.Span);
        return new Artwork(
            Url: cdnUrl.Replace("{file_id}", imageId),
            Width: image.HasWidth ? image.Width : Option<int>.None,
            Height: image.HasHeight ? image.Height : Option<int>.None,
            Size: image.HasSize ? ToSize(image.Size) : Option<ArtworkSizeType>.None
        );
    }
    ...
    public int CompareTo(SpotifyTrackAlbum other)
    {
        /*
        #0055b7
        19FACT
    ...
    public int CompareTo(object? obj)
    {
        ...
        if (obj is SpotifyTrackAlbum other) return CompareTo(other);
        throw new ArgumentException($"Object must be of type {nameof(SpotifyTrackAlbum)}");
    }
