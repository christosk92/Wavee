...
namespace AesCtrNative
{
    public class Aes128CtrTransform
    {
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            int blockCount = DivideByRoundUp(inputCount, BlockSizeBytes);
        ...
        private static int DivideByRoundUp(int value, int divisor) => (value + divisor - 1) / divisor;
        private static long DivideByRoundUp(long value, long divisor) => (value + divisor - 1) / divisor;
