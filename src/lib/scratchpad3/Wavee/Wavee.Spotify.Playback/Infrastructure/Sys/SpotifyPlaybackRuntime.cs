...
public static class SpotifyPlaybackRuntime
{
    ...
    public static async ValueTask<Stream> StreamAudio(this ISpotifyClient client, SpotifyId itemId,
    ...
    private static Aff<RT, Stream> StreamAudio<RT>(ISpotifyClient client, SpotifyId itemId,
        SpotifyPlaybackConfig config)
        ...
        from subFile in ExtractSubFile<RT>(decryptedStream.Stream, decryptedStream.NormalisationDatas, isVorbis)
        select (Stream)subFile;
    ...
    private static Eff<Subfile<RT>> ExtractSubFile<RT>(DecryptedSpotifyStream<RT> decryptedStream,
        ...
        where RT : struct, HasHttp<RT>
    {
        return SuccessEff(new Subfile<RT>(decryptedStream, normData, isVorbis ? SPOTIFY_OGG_HEADER_END : 0));
    }
...
internal readonly record struct TrackOrEpisode(Option<Track> Track, Option<Episode> Episode)
{
