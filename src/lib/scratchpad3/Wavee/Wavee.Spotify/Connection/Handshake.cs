    ...
    where RT : struct, HasTCP<RT>
{
    ...
    internal static Aff<RT, SpotifyEncryptionRecord> PerformClientHello(NetworkStream stream) =>
        from ct in cancelToken<RT>()
        let keys = DhLocalKeys.Random()
        let clientHello = BuildClientHello(keys)
    ...
    private static Aff<RT, (ReadOnlyMemory<byte>, ReadOnlyMemory<byte>)> ReadApResponse(NetworkStream stream) =>
