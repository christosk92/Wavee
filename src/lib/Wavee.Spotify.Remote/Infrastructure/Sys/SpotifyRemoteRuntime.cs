    ...
    HasHttp<RT>
{
    public static Aff<RT, ISpotifyRemoteClient> Connect(
        ...
        from connectionId in GetConnectionId(ws)
        from newRemoteState in Eff(() => SpotifyRemoteState.CreateNew(
            deviceId,
            deviceName,
            deviceType,
            connectionId))
        from cluster in PutState(spClientUrl,
            PutStateReason.NewDevice,
            false,
            newRemoteState, getBearer)
        from cl in Eff<RT, SpotifyRemoteClient<RT>>(rt => new SpotifyRemoteClient<RT>(spClientUrl, newRemoteState,
            cluster, getBearer, rt))
        from _ in SetupListeners(spotifyClient, player, ws, cl)
        select (ISpotifyRemoteClient)cl;

    private static Aff<RT, string> GetConnectionId(WebSocket ws) =>
        from c in Ws<RT>.Read(ws)
        select ParseConnectionId(c);

    private static string ParseConnectionId(ReadOnlyMemory<byte> c)
    {
    ...
    private static Aff<RT, Unit> SetupListeners(
        ISpotifyClient spotifyClient,
        IWaveePlayer player,
        WebSocket ws,
    ...
    private static async Task ProcessMessages(
        ISpotifyClient spotifyClient,
        IWaveePlayer player,
        WebSocket ws, SpotifyRemoteClient<RT> client, RT rt)
    {
        while (true)
        {
            var msgResult =
            ...
            await HandleSpotifyMessage(
                spotifyClient,
                player, spotifyMessage, ws, client, rt);
        }
    }


    private static async Task HandleSpotifyMessage(
        ISpotifyClient spotifyClient,
        IWaveePlayer player,
        SpotifyWebsocketMessage spotifyMessage, WebSocket ws,
        SpotifyRemoteClient<RT> client, RT rt)
    {
            ...
            case SpotifyWebsocketMessageType.Request:
                ...
                var requestResult =
                    await client.OnRequest(
                            spotifyClient,
                            player, key, spotifyRequestCommand)
                        .Run(rt);
    ...
    internal static Aff<RT, Cluster> PutState(
        string spClientUrl,
        PutStateReason reason,
        bool isActive,
        SpotifyRemoteState remoteState,
        Func<Task<string>> getBearer) =>
        from putStateRequest in Eff(() => remoteState.BuildPutState(reason, isActive))
