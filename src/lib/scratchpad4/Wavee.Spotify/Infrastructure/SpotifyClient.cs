...
{
    public static async Task<ISpotifyCore> Create(
        ...
        CancellationToken ct = default)
    {
        ...
        var aff =
            from info in SpotifyClientRuntime<WaveeRuntime>.AuthenticateWithoutConnectionId(
            ...
            from dealer in AP<WaveeRuntime>.FetchDealer()
            from remoteConnection in SpotifyRemoteRuntime<WaveeRuntime>.Create(
                spClientUrl,
                dealer.Host,
                dealer.Port,
                deviceId,
                config.Remote.DeviceName,
                config.Remote.DeviceType,
                () => connection.Token.GetToken(ct),
                ct
            )
            select (connection, remoteConnection);

        var result = await aff.Run(WaveeCore.Runtime);

        var r = result.ThrowIfFail();
        return new SpotifyCore<WaveeRuntime>(r.connection, r.remoteConnection);
    }
}
