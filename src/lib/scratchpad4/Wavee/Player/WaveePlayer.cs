...
public static class WaveePlayer
{
    ...
    public static async ValueTask<IDisposable> Play(IAudioStream stream, Action<IWaveePlayerState> stateChanged,
        CancellationToken ct = default)
    {
        var stateChangedObserver =
            WaveePlayerRuntime<WaveeRuntime>.State.OnChange()
                .Subscribe(stateChanged);
        var aff = WaveePlayerRuntime<WaveeRuntime>.Play(stream, None, None, ct);
...
public readonly record struct StoppedPlayerState(DateTimeOffset Since, Option<IPlayingItem> Item) : IWaveePlayerState;