...
internal static class SpotifyStreams<RT> where RT : struct, HasHttp<RT>,  HasDatabase<RT>
{
        ...
        TrackOrEpisode metadata)
    {
        ...
        return SuccessAff(
            (IEncryptedSpotifyStream)new EncryptedSpotifyStream<RT>(cachedFile.Data,
    ...
    public static LanguageExt.Aff<RT, IEncryptedSpotifyStream> OpenEncryptedStream(
        ...
        select (IEncryptedSpotifyStream)new EncryptedSpotifyStream<RT>(firstChunkMemory,
            numberOfChunks, totalSize,
    ...
    public static LanguageExt.Eff<RT, (DecryptedSpotifyStream<RT> Stream, LanguageExt.Option<NormalisationData>
            NormalisationDatas)>
        OpenDecryptionStream(
            IEncryptedSpotifyStream encryptedSpotifyStream, LanguageExt.Either<AesKeyError, AudioKey> key,
            bool isVorbis)
    {
        var decryptionStream = new DecryptedSpotifyStream<RT>(encryptedSpotifyStream, key);
        var normData = isVorbis
            ? NormalisationData.ParseFromOgg(decryptionStream)
            : LanguageExt.Option<NormalisationData>.None;
        return SuccessEff((decryptionStream, normData));
    }

    public static LanguageExt.Eff<SpotifyStream<RT>> ExtractFinalStream(DecryptedSpotifyStream<RT> decryptedStream,
        LanguageExt.Option<NormalisationData> normData,
        bool isVorbis,
        AudioFile chosenFile,
        TrackOrEpisode metadata,
        LanguageExt.Option<string> uid, 
        Func<TrackOrEpisode, ITrack> mapper)
    {
        return SuccessEff(new SpotifyStream<RT>(decryptedStream,
            ...
            chosenFile, uid, mapper(metadata)));
    }
