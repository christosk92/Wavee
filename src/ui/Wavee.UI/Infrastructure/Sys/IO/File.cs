...
namespace Wavee.UI.Infrastructure.Sys.IO;
    ...
    where RT : struct, HasFile<RT>
{
    ...
    /// </summary>
    [Pure, MethodImpl(AffOpt.mops)]
    public static Eff<RT, ReadOnlyMemory<byte>> readAllBytesSync(string path) =>
        from rs in default(RT).FileEff.Map(e => e.ReadAllBytesSync(path))
        select rs;
