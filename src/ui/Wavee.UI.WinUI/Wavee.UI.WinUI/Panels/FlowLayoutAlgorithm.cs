...
internal class FlowLayoutAlgorithm
{
    ...
    private Rect RealizationRect => IsVirtualizingContext ? _context!.RealizationRect : new Rect(0, 0, Infinity, Infinity);
    ...
    private void SetLayoutOrigin()
    {
        if (IsVirtualizingContext)
        {
            _context!.LayoutOrigin = new global::Windows.Foundation.Point(_lastExtent.X, _lastExtent.Y);
        }
    }
