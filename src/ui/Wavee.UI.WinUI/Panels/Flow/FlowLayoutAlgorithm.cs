...
namespace Wavee.UI.WinUI.Panels.Flow
{
    internal class FlowLayoutAlgorithm
    {
        ...
        private static Rect Infinity = new Rect(0, 0, double.PositiveInfinity, double.PositiveInfinity);
        private Rect RealizationRect => IsVirtualizingContext ? _context!.RealizationRect : Infinity;
            ...
            string? layoutId)
        {
            ...
            {
                _reflowCount = 0; // reset counter if reflow is not required
            }
            RaiseLineArranged();
...
public static class RectExtensions2
{
    public static Rect Translate(this Rect rect, Vector vector)
    {
        return new Rect(rect.X + vector.X, rect.Y + vector.Y, rect.Width, rect.Height);
    }
    //Position
    public static Vector Position(this Rect rect)
    {
