...
public static class DynamicDataExtensions
{
    ...
    public static IDisposable RefillFrom<TObject, TKey>(this ISourceCache<TObject, TKey> sourceCache,
        IObservable<Task<IEnumerable<TObject>>> contents) where TKey : notnull
    {
        return contents
            .Select(async task =>
            {
                var result = await task;
                sourceCache.Edit(updater => updater.Load(result));
            })
            .Subscribe();
    }
}
