...
/// </summary>
public sealed class ScopedStream<T> : IReadBytes, ISeekBuffered where T : IReadBytes, ISeekBuffered
{
    private T _inner;
    ...
    public ulong SeekBuffered(ulong pos)
    {
        ...
        return _inner.SeekBuffered(Math.Min(Math.Max(_start, pos), _start + _len));
    }
